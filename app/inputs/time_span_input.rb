...
class TimeSpanInput < MultiValueInput
  ...
  protected
    ...
    # The markup here is also duplicated in
    # app/assets/javascripts/templates/editor/time_span.hbs.  Any
    # changes to this markup should also be reflected there as well
    def build_components(attribute_name, value, index, options)
      ...
      out << template.label_tag(field_name,
                                field.to_s.humanize,
                                required: false)
      out << "  </div>"
      ...
      out << @builder.text_field(
        field_name,
        options.merge(value: field_value,
                      name: field_name,
      ...
      out << template.select_tag(
        field_name,
        template.options_for_select(time_span_qualifier_options,
                                    field_value),
        include_blank: true,
        label: "",
    ...
    def build_options_for_existing_row(_attribute_name, _index, value, options)
      options[:value] = value.rdf_label.first ||
                        "Unable to fetch label for #{value.rdf_subject}"
    end
