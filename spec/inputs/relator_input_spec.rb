...
describe RelatorInput, type: :input do
  ...
  describe "#build_field" do
    ...
    context "for a resource" do
      let(:value) { double("value 1", rdf_label: ["Item 1"], rdf_subject: "http://example.org/1", node?: false, predicate: :programmer) }
      it "renders multi-value" do
    ...
    context "for an ActiveFedora object" do
      ...
      it "renders multi-value" do
        ...
        expect(subject).to(
          have_selector('input[name="image[creator_attributes][0][predicate]"][value="actor"]',
                        visible: false)
        )
      end
    end
