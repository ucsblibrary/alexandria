...
describe Importer::Cylinder do
  ...
  let(:collection_attrs) do
    { identifier: ["some_ark"], title: ["Cylinders Collection"] }
  end

  let(:collection) do
    Importer::Factory::CollectionFactory.new(
      Importer::Cylinder::COLLECTION_ATTRIBUTES.merge(collection_attrs)
    ).find_or_create
  end
  ...
  describe "#attributes" do
    ...
    it "parses the attributs from the MARC file" do
      ...
      expect(subject["accession_number"]).to(
        eq ["Cylinder 4373", "Cylinder 4374", "Cylinder 4377"]
      )
      ...
      expect(subject["form_of_work"]).to(
        eq ["Musical settings",
            "Sound recordings",
            "Songs and music",
            "Humorous monologues",]
      ...
      expect(subject["table_of_contents"]).to(
  ...
  describe "when the collection doesn't exist" do
    ...
    it "raises an exception" do
      expect { importer.run }.to(
        raise_error(
          CollectionNotFound,
          'Not Found: Collection with accession number ["Cylinders"]'
        )
      )
  ...
  describe "import records from MARC files" do
    ...
    it "imports the records" do
      ...
      record1 = AudioRecording.where(
        Solrizer.solr_name("system_number", :symbol) => "002556253"
      ).first

      record2 = AudioRecording.where(
        Solrizer.solr_name("system_number", :symbol) => "002145837"
      ).first

      record3 = AudioRecording.where(
        Solrizer.solr_name("system_number", :symbol) => "002145838"
      ).first
      ...
      expect(record2.file_sets.map(&:title).map(&:first)).to(
        contain_exactly("Cylinder0006", "Cylinder12783")
      )
      ...
      expect(record1.accession_number).to(
        contain_exactly("Cylinder 4373", "Cylinder 4374", "Cylinder 4377")
      )
      expect(record1.copyright_status.map(&:class)).to(
        eq [ControlledVocabularies::CopyrightStatus]
      )
      expect(record1.digital_origin).to eq ["reformatted digital"]
      expect(record1.edition).to eq ["[Pathé Stentor (Concert) ed.]"]
      expect(record1.institution.map(&:class)).to(
        eq [ControlledVocabularies::Organization]
      )
      expect(record1.issued.first.label).to eq ["[1903]"]
      expect(record1.issued.first.as_array).to eq 1903
      expect(record1.language.first.rdf_subject).to(
      ...
      expect(record1.location.first.class).to(
        eq ControlledVocabularies::Geographic
      )
      ...
      expect(record1.form_of_work).to(
        contain_exactly(
          "Humorous monologues",
          "Musical settings",
          "Songs and music",
      ...
      expect(record1.sub_location).to(
        eq ["Department of Special Research Collections"]
      )
      expect(record1.table_of_contents).to(
        eq ["The whistling coon Sam Devere, words "\
            "/ Sam Raeburn, music -- sleep, baby, sleep -- "\
            "if it wasn't for the irish and the jews "\
            "William Jerome, words / Jean Schwartz, music",]
      ...
      [
        ...
        expect(contributor.map(&:class).uniq).to(
      ...
      expect(org.foaf_name).to(
        eq "United States. National Guard Bureau. Fife and Drum Corps"
      )
      ...
      [:instrumentalist, :lyricist, :arranger].each do |property_name|
        contributor = record1.send(property_name)
        contributor_id = contributor.first.rdf_label.first.gsub(
          Regexp.new('^.*\/'), ""
        )
  ...
  context "marc file without language" do
    let(:meta_files) do
      [File.join(files_dir, "cylinder_sample_without_language_marc.xml")]
    end
